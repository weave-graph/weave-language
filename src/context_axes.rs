...
use weave_contract::quantity::{Quantity, UnitDescriptor};
...
// Deliberately not Deserialize: direct BTreeMap decoding would silently overwrite duplicates.
#[derive(Clone, Debug, Serialize, PartialEq, Eq)]
        ...
        for (name, kind) in &self.schema.axes {
            ...
            let valid = match kind {
                ...
                ContextAxisType::Quantity { unit } => {
                    ...
                    quantity_shape(value)
                        && Quantity::deserialize(value).is_ok_and(|q| q.unit() == unit)
                }
...
// Every object is decoded through this visitor before any map-based typed conversion.
// The input byte ceiling is checked before parsing; collection/depth limits before children.
struct Strict {
    depth: usize,
}
...
impl<'de> Visitor<'de> for Strict {
    ...
    fn visit_map<A: MapAccess<'de>>(self, mut access: A) -> Result<Value, A::Error> {
        ...
        while let Some(key) = access.next_key::<String>()? {
            if key.len() > 512 || object.len() >= MAX_ENUM {
    ...
    fn visit_seq<A: SeqAccess<'de>>(self, mut access: A) -> Result<Value, A::Error> {
        ...
        while let Some(value) = access.next_element_seed(Strict {
            ...
            if values.len() >= MAX_ENUM {
                return Err(serde::de::Error::custom("context array budget"));
            }
            values.push(value);
        }
