...
A bootstrap `lens` produces a graph query. Composing one with an earlier lens combines compatible relation/time filters and inherits metadata options; conflicting filters are rejected. This small filter language is **not yet general graph-valued functions, parameter binding, joins or a rule evaluator**. Each lens declaration emits one query command. Unknown operations, including writes or host authority fields inside a lens, fail parsing.
