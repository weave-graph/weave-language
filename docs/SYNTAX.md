...
Structured property literals support JSON-style objects and arrays, strings, booleans, null, signed integers and finite decimal/exponent numeric literals. Object keys are quoted and unique. Nesting is bounded to 32 levels, within existing source/token budgets. Schema `float` means finite IEEE 754 binary64, including its rounding behavior; it does not mean exact decimal arithmetic. Authored graph interval boundaries require signed 64-bit integer literals. Pure source Interval values and computed Time arguments are described in [exact intervals](INTERVALS.md).
