...
Design only, 2026-09-20. This refines [the approved boundary](REACTOR_ARTIFACTS.md) against language `3ea25e3` and current native dispatch after engine `4e93ae0`. Export work remains independent and takes priority. No protocol number is reserved, no shared DTO is changed and no executable reactor is claimed here.
...
**Recommended initial implementation restriction:** require representable immutable source gates for every input snapshot whose content/existence influences output. Collect exact real source NodeRefs, AssertionRefs and attachment-origin refs, plus existing influence/context carriers, from the authorized input and resolved metadata. Protect the entire result with their conservative AND union even when computation returns empty or scalar-only data. At least one real reference naming each influencing revision must gate that revision's current protected-reference policy; do not infer an equivalent gate solely because two snapshots contain equal data. More gates than the bound is an explicit error, never truncation.

A wholly empty influencing snapshot without a real gate in that revision must reject with E_HANDLER_INFLUENCE after authorization. Do not fabricate an assertion for the event, drop the empty input, or claim another revision's carrier proves this one. This is an implementation-ready restricted profile, **not completion of empty-event semantics**. A later exact snapshot/event influence carrier must close this requirement, and should be reviewed as a separate shared contract change. Parent/engine must accept this restriction or choose that carrier before implementation starts.
...
Compiler/shared validation: at most 16 total view+handler artifacts, 1 MiB per artifact/4 MiB combined, 1,000 total expression nodes per recipe, depth32, bounded identifier/source-manifest sizes, and no duplicate/forward bindings. Native: one event/input/recipe/output per prepare, one stored commit per complete, existing cumulative read/materialization limits, at most 999 unique assertion+node gates before adding any materialization self-pin, and a 16 MiB complete stored Program cap. Recheck per-record proof limits after protection. Preparation metadata+stored bytes need explicit per-principal count/byte quotas and bounded pre-decode SQL loads. New storage tables require a reviewed migration/old-binary compatibility decision and unwind/process-death tests.
