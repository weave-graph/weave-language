...
Proposed compiler `compile_artifacts` returns `{ program, view_templates }`; existing `compile`/`plan`, `specialize`, `fingerprint` and `values` APIs/commands must reject templates with `E_HOST_ARTIFACT_REQUIRED` instead of silently discarding them. This applies to linked-module equivalents too. A future complete artifact-specialization API may include typed scalar values, all artifacts and a versioned aggregate fingerprint; the first implementation does not overload the existing scalar/Program fingerprint to omit artifacts. A new CLI `weave view-plan FILE --template Active` emits one validated template and performs no registration or network operation. Preserve the existing `describe` result shape; introduce separately versioned artifact descriptions or an explicit artifact-required diagnostic. `check` may validate and explicitly report artifact requirements. Pure imports remain declaration-only; exported/imported view templates are a later profile.

Engine supplies a separate native `register_compiled_view(instance_id, template, tick, host)` or equivalent versioned overload. It validates/recomputes the digest, rejects conflicting manifests, persists the manifest and digest atomically with the definition, and conflict-checks/merges those source identities into every result and its fingerprint: initial evaluation, full recomputation, fallback and incremental rematerialization, including empty values. The result cannot silently acquire two digests for one source label/revision. Legacy `register_view` remains compatible, but a legacy source-less registration cannot satisfy a source definition digest merely by matching an expression. Re-registering the same instance with a different definition fails. No serialized HostContext, trusted clock, write grant, policy installation, enrollment or scheduler instruction is permitted in this artifact.
