...
The compiler does not register a view, schedule refresh, subscribe to events or promise push/incremental watch delivery. Protocol 0.16 adds [separate compiled registration artifacts and current graph reads](VIEW_ARTIFACTS.md); the host still performs registration and refresh explicitly. `pin` is not `watch`. Standalone query expressions can be supplied by a trusted host to the existing view API; a program-local Reference is not independently resolvable as a view definition. Adapter/subscription syntax, checkpoints, effect types and source-level watch registration remain L17/L24 gaps.
